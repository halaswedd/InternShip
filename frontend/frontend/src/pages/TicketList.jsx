  ...
  const filtered = useMemo(() => {
    return tickets.filter((t) => {
      const matchesSearch =
        ...
        (t.requested_by || "").toLowerCase().includes(search.toLowerCase());
      ...
      <div className="tl-content">
        <div className="tl-header">
          <div>
            ...
            <p>Manage and monitor internal support requests</p>
          </div>
          <Link to="/create-ticket" className="tl-create-btn">+ Create New Ticket</Link>
        </div>
          ...
          <table>
            ...
            <tbody>
              {paginated.map((ticket) => (
                <tr key={ticket.id}>
                  <td>
                    <Link to={`/tickets/${ticket.id}`} className="tl-ref">{ticket.reference_no}</Link>
                  </td>
                  <td className="tl-subject">{ticket.title}</td>
                  <td>
                    <div className="tl-user">
                      <span className="tl-avatar" style={{ background: avatarColor(ticket.requested_by) }}>
                  ...
                  <td>
                    <div className="tl-actions">
                      <Link to={`/tickets/${ticket.id}`} title="Edit">
                        <Pencil size={15} />
                      </Link>
                      <button onClick={() => handleDelete(ticket.id)} title="Delete">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </td>
              ...
              {paginated.length === 0 && (
                <tr><td colSpan="8" style={{ textAlign: "center", padding: 30 }}>No tickets found.</td></tr>
              )}
            </tbody>
          </table>

          <div className="tl-pagination">
            <span>Showing {paginated.length === 0 ? 0 : (page - 1) * PAGE_SIZE + 1} to {Math.min(page * PAGE_SIZE, filtered.length)} of {filtered.length} tickets</span>
            <div>
