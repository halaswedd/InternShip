...
function Dashboard() {
    ...
    const fetchStats = async () => {
      ...
      try {
        const response = await fetch(
          "https://affectionate-freedom-production-e166.up.railway.app/get_dashboard_stats.php",
          {
            method: "GET",
            headers: {
  ...
  const groupedCategory = {
    ...
    Others:
      (byCategory["Email"] || 0) +
      (byCategory["Access Request"] || 0) +
      (byCategory["Other"] || 0),
  };

  const categoryTotal =
    Object.values(groupedCategory).reduce((a, b) => a + b, 0) || 1;

  let cumulative = 0;

  const gradientParts = Object.entries(groupedCategory).map(
    ([label, count]) => {
      ...
      <div className="db-page">
        <div className="db-header">
          <div>
            ...
            <p>
              You have {openCount} tickets requiring immediate attention today.
            </p>
          </div>

          <Link to="/create-ticket" className="db-create-btn">
            + Create Ticket
          </Link>
        </div>
        ...
        <div className="db-widgets">
          <div className="db-widget">
            <div className="db-widget-icon blue">
              <Ticket size={18} />
            </div>

            <span className="db-widget-number">
          ...
          <div className="db-widget">
            <div className="db-widget-icon gray">
          ...
          <div className="db-widget">
            <div className="db-widget-icon green">
                ...
                {Object.entries(groupedCategory).map(([label, count]) => (
                  <div className="db-legend-row" key={label}>
                    <span
                      ...
                      style={{ background: CATEGORY_COLORS[label] }}
                    ></span>
                ...
                return (
                  <div className="db-priority-row" key={label}>
                    <div className="db-priority-top">
                      ...
                      <span style={{ color: PRIORITY_COLORS[label] }}>
                        {count}
                      </span>
                    </div>
                    ...
                    <div className="db-priority-track">
                      <div
                        ...
                        style={{
                          ...
                          background: PRIORITY_COLORS[label],
                        }}
                      />
                    </div>
                  </div>
        ...
        <div className="db-panel">
          <div className="db-panel-header">
            ...
            <Link to="/tickets" className="db-view-all">
              View All
            </Link>
          </div>
            ...
            <tbody>
              {(stats?.recent_tickets || []).map((t) => (
                <tr key={t.id}>
                  ...
                  <td>
                    <span
                      className={`db-status-badge status-${t.status
                        .toLowerCase()
                        .replace(" ", "-")}`}
                    >
                      {t.status}
      ...
      <footer className="db-footer">
        <div className="lp-footer-left">
          <span className="lp-footer-brand">
            IT
            <span className="lp-footer-brand-accent">HelpDesk</span>
          </span>
